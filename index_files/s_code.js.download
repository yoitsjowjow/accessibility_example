...
var s_account = "gmigibsoncomprod"
...
s.linkInternalFilters = "javascript:,gibson.com,gibsoncustom.com,gibsoneurope.com,gibsonguitar.in,gibsonguitar.es,10.1.50.1;localhost;epiphone.com;kramerguitars.com"
...
function s_doPlugins(s) {
    ...
    if (!s.prop1) s.prop1 = s.getQueryParam('q');

    /*socialPlatforms v1.0 - used for SocialAnalytics*/
    s.socialPlatforms('eVar12');
    ...
    /*	START GIBSON EDITS FOR PAGENAME	*/
    tempPageName = s.pageName;
    tempPageName = unescape(tempPageName).toLowerCase();
    tempPageName = tempPageName.replace(/ /g, "_");
    tempPageName = tempPageName.replace(/</g, "_");
    tempPageName = tempPageName.replace(/>/g, "_");
    tempPageName = tempPageName.replace(/#/g, "_");
    tempPageName = tempPageName.replace(/%/g, "_");
    ...
    for (var x = 0; x < tempPageName.length; x++) {
        if (tempPageName.substring(x, x + 1) == "_")
        { strpos = x; }
        else
        { break; }
    }

    tempPageName = tempPageName.substring(strpos + 1, tempPageName.length);

    strpos = tempPageName.length;
    for (var x = tempPageName.length; x > -1; x--) {
        if (tempPageName.substring(x - 1, x) == "_")
        { strpos = x; }
        else
        { break; }
    }

    s.pageName = tempPageName.substring(0, strpos - 1);

    /*	END GIBSON EDITS FOR PAGE NAME */

    if (s.pageName) s.eVar4 = s.pageName;
    if (s.channel) s.eVar5 = s.channel;

    /* Plugin Example: timeparting EST */
    s.prop8 = s.getTimeParting('h', '-5', '2007'); // Set hour 
    s.prop9 = s.getTimeParting('d', '-5', '2007'); // Set day
    s.prop10 = s.getTimeParting('w', '-5', '2007'); // Set Weekend / Weekday


    /* Plugin Example: linkHandler v0.5  */
    s.linkLeaveQueryString = true;
    var exiturl = s.exitLinkHandler();
    var url = s.linkHandler('mailto:', 'o');
    //if link clicked was a mailto link
    if (url) {
        // save pre-linkHandler variable values
        s.eVar6_original = s.eVar6;
        s.ltv_original = s.linkTrackVars;
        s.lte_original = s.linkTrackEvents;
        s.events_original = s.events;
        ...
        if (s.ltv_original) s.linkTrackEvents = s.lte_original;
        if (s.eVar6_original) s.eVar6 = s.eVar6_original;
        if (s.events_original) s.events = s.eVents_original;
    }
...
+ "'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar11';s.Integrate"
...
s.visitorNamespace = "gibsonmusicalinstruments"
s.trackingServer = "stats.gibson.com"
s.trackingServerSecure = "sstats.gibson.com"
s.dc = 112
